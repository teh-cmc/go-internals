...
\(_NOTE: In assembly outputs, these wrapper methods are marked as _`<autogenerated>`_._\)
...
We can prove that with a few lines of code \([escape.go](https://github.com/teh-cmc/go-internals/tree/27fcccf8e8ba6a009989ac199f57d2502e867b8c/chapter2_interfaces/escape.go)\):
...
_NOTE: For the remainder of this chapter, we will denote an interface _`I`_ that holds a type _`T`_ as _`<I,T>`_. E.g. _`Mather(Adder{id: 6754})`_ instantiates an _`iface<Mather, Adder>`_._
...
Consider the following program \([compound\_interface.go](https://github.com/teh-cmc/go-internals/tree/27fcccf8e8ba6a009989ac199f57d2502e867b8c/chapter2_interfaces/compound_interface.go)\):
...
On an unrelated note, this little program demonstrates something that we had not seen up until now: since the generated `itab` is specifically tailored to _a pointer to_ a `Constructor`, as opposed to a concrete value, this fact gets reflected both in its symbol-name \(`go.itab.*"".Calculator,"".Mather`\) as well as in the `_type` that it embeds \(`type.*"".Calculator`\).  
...
Consider this short program \([eface\_to\_type.go](https://github.com/teh-cmc/go-internals/tree/27fcccf8e8ba6a009989ac199f57d2502e867b8c/chapter2_interfaces/eface_to_type.go)\):
...
Type-switches are a bit trickier, of course. Consider the following code \([eface\_to\_type.go](https://github.com/teh-cmc/go-internals/tree/27fcccf8e8ba6a009989ac199f57d2502e867b8c/chapter2_interfaces/eface_to_type.go)\):
...
It's easy to confirm this with a bunch of benchmarks \([eface\_to\_type\_test.go](https://github.com/teh-cmc/go-internals/tree/27fcccf8e8ba6a009989ac199f57d2502e867b8c/chapter2_interfaces/eface_to_type_test.go)\):
...
Speaking of type hashes, how is it that we know that `$-800397251` corresponds to `type.uint32.hash` and `$-269349216` to `type.uint16.hash`, you might wonder? The hard way, of course \([eface\_type\_hash.go](https://github.com/teh-cmc/go-internals/tree/27fcccf8e8ba6a009989ac199f57d2502e867b8c/chapter2_interfaces/eface_type_hash.go)\):
...
* [Issue \#24631 \(golang/go\): _testing: don't truncate allocs/op_](https://github.com/golang/go/issues/24631)
